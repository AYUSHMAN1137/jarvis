...
class RealtimeGroqService(GroqService):
    ...
    def search_tavily(self, query: str, num_results: int = 5) -> str:
        ...
        try:
            monitor = get_api_key_monitor()
            monitor.record_provider_attempt("tavily", operation="search", source="realtime_service")
            t0 = time.perf_counter()
            response = with_retry(
            ...
            if not results and not ai_answer:
                ...
                monitor.record_provider_success("tavily", operation="search", source="realtime_service")
                return ("", None)
            ...
            get_api_key_monitor().record_provider_failure("tavily", operation="search", source="realtime_service", error=str(e))
