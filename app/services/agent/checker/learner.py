    ...
    def _retry(self, tool: str, args: dict) -> str:
        ...
        try:
            ...
            coordinator = ExecutionCoordinator(
                registry=self.registry, phase4=None
            )
            context = ExecutionContext(
                user_message=f"verified retry: {tool}", source="learner_retry"
            )
            action = coordinator.action(tool, args, index=1)
            # Learner reaches this path only for tools already classified safe;
            # the coordinator still re-reads current registry risk metadata.
            manifest = coordinator.execute_plan(context, [action])
            return manifest.results[0].observation if manifest.results else ""
        except Exception as e:  # noqa: BLE001
