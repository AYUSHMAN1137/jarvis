...
class UserModel:
    ...
    def aggregate_from_provider(self) -> int:
        """Pull recent actions from the injected provider and aggregate."""
    ...
    def forget_all(self) -> bool:
        ...
        try:
            with self._lock:
                for tbl in ("um_facts", "um_aliases", "um_habits"):
...
def _default_action_provider() -> List[dict]:
    ...
    try:
        ...
        if conn is None:
            return []
        rows = conn.execute(
            "SELECT tool, target, args, ok, created_at FROM actions ORDER BY id ASC"
        ).fetchall()
        return [{"tool": r[0], "target": r[1], "args": r[2], "ok": r[3],
